...
`runAgentNudge` applies it right before the customer post (after the assignee re-check): `freeform` → `sendMessage`; `template` → `buildTemplatePayload` → `client.sendTemplate` (HSM); `note` → `sendPrivateNote` (the agent's intended message is kept internally — never a free-form message WhatsApp would reject). New outcome: `templated`.
