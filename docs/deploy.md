...
> **`20260831153000_payment_appointment_chain` also changes a conversion unique key.** Stop the old process before `migrate deploy`, then start only the new image. The migration backfills the new non-null `external_id` from the old conversation identity before replacing `(tenant_id, thread_id, source)` with `(tenant_id, source, external_id)`. Running an old writer after that replacement would omit the now-required column; the ordered stop/migrate/start swap prevents that mixed-version write.
