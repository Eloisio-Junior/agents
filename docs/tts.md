...
**Format matters: WhatsApp only recognizes a voice note (PTT) when the audio is Ogg/Opus** — mp3/wav arrive as a plain file attachment. Both providers emit Opus natively (OpenAI `response_format:"opus"`; ElevenLabs `output_format=opus_48000_64`), so there is no server-side transcode. See `src/modules/tts/providers.ts`.
